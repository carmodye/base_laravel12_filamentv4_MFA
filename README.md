...
decide if you want to use .devcontainer with sail or native or sail without devcontainer
and adjust commands 
Here are the steps to install Laravel Sail with a devcontainer setup:
...
Inside .devcontainer, add a devcontainer.json file with content like:
{
  "name": "Laravel Sail",
...
Open Command Palette (F1), select “Dev Containers: Rebuild and Reopen in Container”.
Start Sail:

In the devcontainer terminal, run:
./vendor/bin/sail up -d
You now have Laravel Sail running inside a devcontainer, with your chosen extensions installed.

should be able to start vsc with
code . and it will prompt to open in devconatiner
...
if in devcontainer or native don't use 
sail 
